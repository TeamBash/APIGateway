...
docker ps -a | grep 'apigateway-service' | awk '{print $1}' | xargs --no-run-if-empty docker stop
docker ps -a | grep 'apigateway-service' | awk '{print $1}' | xargs --no-run-if-empty docker rm
